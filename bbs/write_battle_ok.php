 ...
 **************************************************************************/
	if($mode=="modify"&&$no) {
		...
		// 일반->일반, 공지->공지 일때 
			...
			@mysql_query("update $t_board"."_$id set name='$name',subject='$subject',email='$email',homepage='$homepage',memo='$memo',sitelink1='$sitelink1',sitelink2='$sitelink2',x='$x',y='$y',use_html='$use_html',reply_mail='$reply_mail',is_secret='$is_secret',category='$category' $del_que1 $del_que2 where no='$no'") or error("4".mysql_error()); 
...
/*
		...
		if($termOfDay >1800){
			if($werewolfSetup['24h-town'] == 2) Error("더 이상 마을을 만들 수 없습니다.");
			else mysql_query("update $DB_setup set `24h-town`=".($werewolfSetup['24h-town'] +1 )."  where name='basic'");
		}
		else{
			if($werewolfSetup['30m-town'] == 1) Error("더 이상 마을을 만들 수 없습니다.");
			else mysql_query("update $DB_setup set `30m-town`=".($werewolfSetup['30m-town'] +1 )."  where name='basic'");
		}
*/
...
//게임 정보 입력
	if($mode=="modify"&&$no) {
		...
		@mysql_query(
		"INSERT INTO `$t_board"."_$id"."_gameinfo` ( `game` , `turn` ,`round` , `winner` ) VALUES ('$no', '0', '0', '0');") or error("7".mysql_error());
