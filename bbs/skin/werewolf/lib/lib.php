...
function RPSetselect($sort) {
	$result = mysql_query("select * from $DB_characterSet where is_use = 1 order by $sort");
	while($temp = mysql_fetch_array($result))
...
//°ÔÀÓÀÌ Á¾·áµÇ¾ú´Ù¸é ±â·ÏÀ» ³²±ä´Ù.
	$noCommentPlayer_list=mysql_query("select * from $DB_entry where game = $no and alive='»ýÁ¸' and comment = 0 and victim = 0 ");

	if($noCommentPlayer_list){
	fwrite($file,"[ÇÑ¹øµµ µ¡±ÛÀ» ³²±âÁö ¾ÊÀº »ç¶÷ÀÌ ÀÖ´Ù¸é ºñ¸Å³Ê ÇÃ·¹ÀÌ¾î·Î ±â·ÏÇÑ´Ù.]------------ \n");
	...
	if($noCommentPlayer_list){
		while($noCommentPlayer = mysql_fetch_array($noCommentPlayer_list)){
			$sql ="update `$DB_entry` set `suddenCount` = `suddenCount` + 1 where `game` = '$no' and `character` = '$noCommentPlayer[character]';";
				fwrite($file,"\$sql:".$sql." \n"); 
				@mysql_query($sql) or die("µ¹¿¬»ç ±â·Ï À» ÀÔ·Â Áß¿¡ ¿À·ù°¡ ¹ß»ýÇß½À´Ï´Ù.");		
		}
	}

	$noCommentPlayer_list=mysql_query("select * from $DB_entry where game = $no and alive='»ýÁ¸' and `suddenCount` = $MaxSuddenCountUnder30M and victim = 0 ");
...
//°ÔÀÓÀÌ Á¾·áµÇ¾ú´Ù¸é ±â·ÏÀ» ³²±ä´Ù.
	...
	fwrite($file,"[°ÔÀÓ µ¿¾È ºñ¸Å³Ê ÇàÀ§¸¦ ÇÑ »ç¶÷µéÀ» ¼Ò¸í °Ô½ÃÆÇ¿¡ °Ô½ÃÇÑ´Ù.]\n");
		while ($noMannerPlayer=mysql_fetch_array($noMannerPlayer_list)){
			...
			$subject  = "[".$game_list[$noMannerPlayer['game']]."] ".$noMannerPlayer['name'].'´Ô ¼Ò¸íÇØ ÁÖ¼¼¿ä.';
			$memo = $character_list[$noMannerPlayer['character']].' ('.$truecharacter_list[$noMannerPlayer['truecharacter']].') ·Î ÀÖÀ¸¸é¼­ '.$noMannerPlayer['deathday'].' ÀÏ Â°¿¡ µ¹¿¬»ç ÇÏ¼Ì½À´Ï´Ù.\n\n';
			$memo .= 'ÀÌ¿¡ °°ÀÌ ÇÃ·¹ÀÌÇÑ ¸¶À» »ç¶÷µé¿¡°Ô ¼Ò¸íÇØ ÁÖ¼¼¿ä.';
...
*/
	$sql = "SELECT  count(*) FROM  `zetyx_board_werewolf_entry` ,`zetyx_board_werewolf_gameinfo` where player = $member[no] and `zetyx_board_werewolf_entry`.game = `zetyx_board_werewolf_gameinfo`.game and (state = '°ÔÀÓ³¡' or state = '¹ö±×' or state = 'Å×½ºÆ®')";
	$playCount =  mysql_fetch_array(mysql_query($sql));
	$playCount = $playCount[0];

}

//$playCount = DB_array("game","game"," `zetyx_board_werewolf_gameinfo`  WHERE state =  '°ÔÀÓ³¡'  ");
...
$NowPlayingGame = DB_array("game","game"," `zetyx_board_werewolf_gameinfo`  WHERE state =  'ÁØºñÁß' OR state =  '°ÔÀÓÁß'");
