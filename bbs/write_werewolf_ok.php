...
//	if(!($is_admin or (1 <= $member['level'] and $member['level'] <= 7 and $playCount >= 10 and $suddenDeathCount == 0))) Error("마을을 만들 수 있는 권한이 없습니다.");
	...
	if($delayAfterS < 0 or $delayAfterS >= $termOfDay) Error("발언 제한 시간(마을 시작 직후)을 다시 입력해주시기 바랍니다. (0에서 ".($termOfDay / 60 - 1)." 사이로 입력)");
	if($delayBeforeS < 0 or $delayBeforeS >= $termOfDay) Error("발언 제한 시간(사건 발생 직전)을 다시 입력해주시기 바랍니다. (0에서 ".($termOfDay / 60 - 1)." 사이로 입력)");
	if(($delayBeforeS + $delayAfterS) >= $termOfDay) Error("발언 제한 시간(마을 시작 직후 & 사건 발생 직전)을 다시 입력해주시기 바랍니다. (총합이 0에서 ".($termOfDay / 60 - 1)." 사이가 되도록 입력)");
