...
// 사용권한 체크
	...
	if($id == "attendWerewolf"){
		if($member[level] <> 8 and $member[level]<> 9) error("신규회원 또는 돌연사 한 사람만 출석도장을 찍을 수 있습니다.");
		if( 2< date('H')  and  date('H') < 20) error("20시에서 02시 사이에 도장을 찍을 수 있습니다.");
		
		$t_today = date("Ymd");
		$check=mysql_num_rows(mysql_query("select no from zetyx_board_$id where subject='$t_today' and ismember='$member[no]'"));
		if($check)error("도장을 이미 찍으셨습니다.");
	}
 ...
 **************************************************************************/
		...
		if($id=="attendWerewolf"){
			if( 2< date('H')  and  date('H') < 20) error("제 시간에 와야죠! 오후 8시(20시) 이후에 오세요!");
			if($member[level] <> 8 and $member[level]<> 9) error("신규회원 또는 돌연사 한 사람만 출석도장을 찍을 수 있습니다.");
		}
