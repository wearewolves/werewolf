 ...
 **************************************************************************/
	if($mode=="modify"&&$no) {
		...
		if(!$notice&&$s_data[headnum]<="-2000000000") {
			...
			@mysql_query("update $t_board"."_$id set headnum='$headnum',prev_no='$prev_no',next_no='$next_no',child='$child',depth='$depth',arrangenum='$arrangenum',father='$father',name='$name',email='$email',homepage='$homepage',subject='$subject',memo='$memo',sitelink1='$sitelink1',sitelink2='$sitelink2',x='$x',y='$y',use_html='$use_html',reply_mail='$reply_mail',is_secret='$is_secret',category='$category' $del_que1 $del_que2 where no='$no'") or error(mysql_error()); 
		...
		// 일반->일반, 공지->공지 일때 
			...
			@mysql_query("update $t_board"."_$id set name='$name',subject='$subject',email='$email',homepage='$homepage',memo='$memo',sitelink1='$sitelink1',sitelink2='$sitelink2',x='$x',y='$y',use_html='$use_html',reply_mail='$reply_mail',is_secret='$is_secret',category='$category' $del_que1 $del_que2 where no='$no'") or error(mysql_error()); 
...
//버그 데이타 입력
	if($mode=="modify"&&$no) {
		@mysql_query("update `$t_board"."_$id"."_brief` set `type`= '$type', `server`='$server' , `serverity`= '$serverity'  where bug='$no';") or die("버그 내용을 입력 중에 오류가 발생했습니다.");
	} elseif($mode=="reply"&&$no) {
		@mysql_query("INSERT INTO `$t_board"."_$id"."_brief` ( `no` , `bug` , `type` , `server` ,  `serverity` , `status` , `dealResult` ,`reporter` , `repairman`,`report_date`,`repair_date` ) VALUES ('', '$no', '$type', '$server',  '$serverity', '1', '','$member[no]', '','$reg_date','$reg_date');") or die("버그 내용을 입력 중에 오류가 발생했습니다.");
	} elseif($mode=="write") {
		@mysql_query(
		"INSERT INTO `$t_board"."_$id"."_brief` ( `no` , `bug` , `type` , `server` ,  `serverity` , `status` , `dealResult` ,`reporter` , `repairman`,`report_date`,`deal_date` ) VALUES ('', '$no', '$type', '$server',  '$serverity', '1', '','$member[no]', '','$reg_date','$reg_date');") or die("버그 내용을 입력 중에 오류가 발생했습니다.");
}
