...
// 마을을 만들 수있는 권한
	require_once("skin/".$id."/class/Player.php");
	require_once("skin/".$id."/class/DB.php");
...
//	$re = mysql_fetch_array(mysql_query("select count(*) as count from  $DB_suddenDeath where player = $member[no]"));
	
//	$suddenDeathCount = $re['count'];

//	if(!($is_admin or (1 <= $member['level'] and $member['level'] <= 7 and $playCount >= 10 and $suddenDeathCount == 0))) Error("마을을 만들 수 있는 권한이 없습니다.");

	if($termOfDay > 1800){
		if(!($is_admin or (1 <= $member['level'] and $member['level'] <= 7 and $player->playCount() >= 1 ))) Error("마을을 만들 수 있는 권한이 없습니다.");
	}

	if($termOfDay <= 1800){
		if(!($is_admin or (1 <= $member['level'] and $member['level'] <= 9 and $member['level'] <> 8 ))) Error("마을을 만들 수 있는 권한이 없습니다.");
	}
	...
	// limit for delayM
	if($delayM * 60 >= $termOfDay) Error("N분룰 시간을 다시 입력해주시기 바랍니다. (".($termOfDay / 60)." 보다 작게 입력)");
 ...
 **************************************************************************/
	if($mode=="modify"&&$no) {
		...
		// 일반->일반, 공지->공지 일때 
			...
			@mysql_query("update $t_board"."_$id set name='$name',subject='$subject',email='$email',homepage='$homepage',memo='$memo',sitelink1='$sitelink1',sitelink2='$sitelink2',x='$x',y='$y',use_html='$use_html',reply_mail='$reply_mail',category='$category' $del_que1 $del_que2 where no='$no'") or error("4".mysql_error()); 
 ...
 **************************************************************************/
		...
		/*
		if(!$is_admin and $termOfDay > 1800){
			$limittime = mktime(0, 0, 0, date("m",time()+86400), date("d",time()+86400), date("Y",time()+86400));
			$limittime = mysql_fetch_array(mysql_query("select count(*) as count from  `zetyx_board_werewolf_gameinfo` where `deathtime` > $limittime and `termOfDay` > 1800" ));
			if($limittime['count'] >= 2) error("더 이상 마을을 만들 수 없습니다.<br>마을은 하루 동안 2개만 만들어 질 수 있습니다. <br>마을이 더 필요할 땐 운영자에게 문의해 주세요.");
		}
		*/

		require_once("skin/werewolf/config/admin_setup.php");

		if( $termOfDay > 1800){
			$playingGame = mysql_fetch_array(mysql_query("select count(*) as count from  `zetyx_board_werewolf_gameinfo` where (`state` like '준비중' or `state` like '게임중') and `termOfDay` > 1800" ));

			if($playingGame['count'] >= $H24GameMaxCount ) error("더 이상 마을을 만들 수 없습니다.<br>24시간 마을은 최대 ".$H24GameMaxCount."개까지만 운영됩니다.");
		}

		if( $termOfDay <= 1800){
			$playingGame = mysql_fetch_array(mysql_query("select count(*) as count from  `zetyx_board_werewolf_gameinfo` where (`state` like '준비중' or `state` like '게임중' ) and `termOfDay` <= 1800" ));

			if($playingGame['count'] >= $M30GameMaxCount ) error("더 이상 마을을 만들 수 없습니다.<br>30분 마을은 최대 ".$M30GameMaxCount."개까지만 운영됩니다.");
		}
		...
		$checktime[0] = $dtime - $rangeOfTime;
		$checktime[1] = $dtime + $rangeOfTime;

		$playGameList =	mysql_query("select deathtime  as count from  `zetyx_board_werewolf_gameinfo` where `state` like '준비중' or  `state` like '게임중'" );
		while($limittime = mysql_fetch_row ($playGameList)){

			$checktime[3] = mktime (date("H",$limittime[0]),date("i",$limittime[0]),0,date("m"),date("d"),date("y"));

			if($checktime[0] < $checktime[3] and $checktime[3] < $checktime[1]) error("시간을 바꿔주세요.<br>비슷한 시간(".date("m월 j일, H시 i분",$limittime[0]).")에 열리는 마을이 있습니다. 30분 이상 차이가 있어야 합니다.");
		}
	...
	if($mode=="modify" && $no) {
		...
		if($gameinfo['state'] == "준비중") {
			...
			if($deathtime != $gameinfo['deathtime']){
				...
				$sql = "select deathtime  as count from  `zetyx_board_werewolf_gameinfo` where (`state` like '준비중' or  `state` like '게임중') and game <> ".$no;

				$playGameList =	mysql_query($sql  );
...
//			@mysql_query("update `$t_board"."_$id"." set `is_secret`= '$is_secret' where no = $no ;") or die("게임 정보를 입력 중에 오류가 발생했습니다.");
		...
		"INSERT INTO `$t_board"."_$id"."_gameinfo` (`game`, `day`, `startingTime`, `deathtime`, `players`, `result`, `state`, `termOfDay`, `characterSet`, `rule`, `subRule`, `useTimetable`, `delay`) VALUES ('$no', '0', '$startingTime', '$deathtime', '1', '', '준비중', '$termOfDay', '$characterSet', '$rule', '$subRule', '$useTimetable', '$delayM');") or error("7".mysql_error());
...
//		$characterSetData=mysql_fetch_array(mysql_query("select * from $DB_characterSet where no = $characterSet "));

//		if($characterSetData[NPC])$character=$characterSetData[NPC];
//		else {
			$temp_character = DB_array("no","no","$DB_character where `set`  =  $characterSet and `comment` != ''  and `greeting` != '' ");
			$temp_character = array_values($temp_character);

			$character= $temp_character[rand (0, count ($temp_character)-1)];
//		}

		@mysql_query(
		"INSERT INTO `$t_board"."_$id"."_entry` ( `no` , `game`,`name` , `player` , `character` ,  `truecharacter`,`victim`,`alive`,`comment`) VALUES ('','$no', 'NPC', '','$character' ,  '1','1', '생존','1' );") or die("참가자 정보를 입력 중에 오류가 발생했습니다.");


		$character_detail=mysql_fetch_array(mysql_query("select * from $DB_character where no = $character"));

		$comment= $character_detail['character']."님이 마을에 도착하셨습니다.";
		$comment=addslashes($comment);
		$reg_date=time();
		// 코멘트 입력
		mysql_query("insert into $t_comment"."_$id (parent,ismember,name,password,memo,reg_date,ip) values ('$no','$member[no]','$member[name]','$password','$comment','$reg_date','$server[ip]')") or error("insert into $t_comment"."_$id (parent,ismember,name,password,memo,reg_date,ip) values ('$no','$member[no]','$member[name]','$password','$memo','$reg_date','$server[ip]')"."8".mysql_error());
		...
		$comment = $character_detail['greeting'];
		$comment=addslashes($comment);
		$reg_date=time();
		// 코멘트 입력
		mysql_query("insert into $t_comment"."_$id (parent,ismember,name,password,memo,reg_date,ip) values ('$no','$member[no]','$member[name]','$password','$comment','$reg_date','$server[ip]')") or error("insert into $t_comment"."_$id (parent,ismember,name,password,memo,reg_date,ip) values ('$no','$member[no]','$member[name]','$password','$memo','$reg_date','$server[ip]')"."10".mysql_error());

		// 코멘트 타입 입력
		$commentID=mysql_insert_id();
		mysql_query("insert into $t_comment"."_$id"."_commentType (game,comment,type,`character`) values ($no,$commentID,'일반','$character')") or error("11".mysql_error());		
