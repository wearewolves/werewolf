<?
  ...
  if(!$s_data[child]) // 답글이 없을때;;
  {
   ...
   mysql_query("update $t_category"."_$id set num=num-1 where no='$s_data[category]'",$connect);
	...
	//버그 기록 삭제
   @mysql_query("delete from $DB_brief where bug=$no;") or die("버그 기록을 삭제 하는 중에 오류가 발생했습니다.");

	//버그 처리 기록 삭제
   @mysql_query("delete from $DB_addnote where parent =$no;") or die("버그 처리 기록을 삭제 하는 중에 오류가 발생했습니다.delete from $DB_bugAddNote where parent =$no");
  ...
  movepage("http://werewolf4.cafe24.com/bbs/zboard.php?id=$id&page=$page&page_num=$page_num&select_arrange=$select_arrange&desc=$des&sn=$sn&ss=$ss&sc=$sc&keyword=$keyword&sn1=$sn1&divpage=$divpage&target=My");
